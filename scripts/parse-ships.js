...
for(const skipper of skipped) {
    ...
    try {
        hourCount = skipper.blocks[0].text.text.match(/Made in (\d+)\s*hours?/)[1]
    } catch (e) {}
    try {
    ...
    if(!repo) {
        console.log(`Repo: `)
     repo = await new Promise(resolve => process.stdin.once('data', data => resolve(data.toString().trim())))
    }
    if(!demo) {
        console.log(`Demo: `)
     demo = await new Promise(resolve => process.stdin.once('data', data => resolve(data.toString().trim())))
    }
    if(!hourCount) {
        console.log(`Hours: `)
     hourCount = await new Promise(resolve => process.stdin.once('data', data => resolve(data.toString().trim())))
    }
    if(!userId) {
        console.log(`UserId: `)
     userId = await new Promise(resolve => process.stdin.once('data', data => resolve(data.toString().trim())))
    }
    final.push({
        userId, repo, demo, hourCount
    })
