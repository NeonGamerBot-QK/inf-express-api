...
async function migrate() {
    ...
    for (const file of fs.readdirSync(path.join(__dirname, "..", "src", "endpoints"))) {
        ...
        const sqliteDb = new Keyv({ store: sqliteStore, namespace: file.split(".")[0].trim() });

        const postgresStore = new KeyvPostgres({
            uri: POSTGRES_URI,
            table: file.split(".")[0].replace(/-/g, '_')
        });
        const postgresDb = new Keyv({ store: postgresStore, namespace: file.split(".")[0].trim() });
        // continue;
        try {
