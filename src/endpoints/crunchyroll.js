...
module.exports = (router, db) => {
    ...
    router.get('/comments/:epid/:epname', rateLimit({ windowMs: 1000, limit: 2 }), (req,res) => {
        // db.get(`${req.params.epid}_${req.params.epname}`)
res.json(
    db.get(`${req.params.epid}_${req.params.epname}`) || []
)
    })
    router.post('/comments/:epid/:epname', rateLimit({ windowMs: 5000, limit: 3 }), (req,res) => {
...
if(typeof user_data !== 'object') errors.push("'user_data' is not an object")
    if(typeof content !== 'string') errors.push("'content' is not a string")
        ...
        const comments =     db.get(`${req.params.epid}_${req.params.epname}`) || []
comments.push({
    user_data, 
    content,
    userId,
